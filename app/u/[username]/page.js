...
export default function PublicProfilePage({ params }) {
  ...
  if (profile.profile_status === 'coming_soon') {
    return (
      <div style={{ minHeight: '100vh', backgroundColor: baseColor, backgroundImage: profile.bg_css || `radial-gradient(at 0% 0%, rgba(0,0,0,0.4) 0px, transparent 50%), radial-gradient(at 100% 100%, rgba(0,0,0,0.3) 0px, transparent 50%)`, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '20px', fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, Helvetica, Arial, sans-serif', color: textColor, position: 'relative', overflowX: 'hidden' }}>
        <style>{`
        ...
        <p style={{ fontSize: '15px', fontWeight: '500', opacity: 0.7, margin: 0, textAlign: 'center', maxWidth: '280px', lineHeight: '1.6' }}>This page is currently being prepared. Please check back soon.</p>

        {profile.tier === 'free' && (
          <footer style={{ position: 'absolute', bottom: '40px', opacity: 0.4, fontSize: '11px', fontWeight: '800', letterSpacing: '2px' }}><a href="/" style={{ color: textColor, textDecoration: 'none' }}>⚡ POWERED BY LINKSUPPLY</a></footer>
        )}
      </div>
    )
  }
  ...
  const handleShare = async () => {
    if (navigator.share) { navigator.share({ title: profile.display_name || profile.username, url: window.location.href }); } 
    else { navigator.clipboard.writeText(window.location.href); alert("Link copied!"); }
  };

  // Dynamic colors for the Aura Glow and Footer based on contrast
  const auraRgb = textColor === 'white' ? '255, 255, 255' : '0, 0, 0';
  const footerBg = textColor === 'white' ? 'rgba(255, 255, 255, 0.1)' : 'rgba(0, 0, 0, 0.04)';
  const footerBorder = textColor === 'white' ? 'rgba(255, 255, 255, 0.15)' : 'rgba(0, 0, 0, 0.1)';
  ...
  return (
    <div style={{ 
      ...
      padding: '60px 20px 120px 20px', // Extra bottom padding to ensure content clears the floating footer
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        /* 🔥 UPGRADE 1: The Silky Cascade Loading */
        @keyframes slideUpFade {
          0% { opacity: 0; transform: translateY(20px); }
          100% { opacity: 1; transform: translateY(0); }
        }
        .cascade-1 { opacity: 0; animation: slideUpFade 0.6s cubic-bezier(0.16, 1, 0.3, 1) 0.1s forwards; }
        .cascade-2 { opacity: 0; animation: slideUpFade 0.6s cubic-bezier(0.16, 1, 0.3, 1) 0.2s forwards; }
        .cascade-3 { opacity: 0; animation: slideUpFade 0.6s cubic-bezier(0.16, 1, 0.3, 1) 0.3s forwards; }
        .cascade-4 { opacity: 0; animation: slideUpFade 0.6s cubic-bezier(0.16, 1, 0.3, 1) 0.4s forwards; }
        .cascade-5 { opacity: 0; animation: slideUpFade 0.6s cubic-bezier(0.16, 1, 0.3, 1) 0.5s forwards; }

        /* 🔥 UPGRADE 2: The Aura Glow Engine */
        @keyframes auraSpin {
          0% { transform: translate(-50%, -50%) rotate(0deg); }
          100% { transform: translate(-50%, -50%) rotate(360deg); }
        }
        .profile-aura-container {
          position: relative; width: 130px; height: 130px; margin-bottom: 22px;
          display: flex; justify-content: center; alignItems: center;
        }
        .profile-aura {
          position: absolute; top: 50%; left: 50%;
          width: 140px; height: 140px; border-radius: 50%;
          background: conic-gradient(from 0deg, transparent 0%, rgba(${auraRgb}, 0.15) 25%, rgba(${auraRgb}, 0.6) 50%, rgba(${auraRgb}, 0.15) 75%, transparent 100%);
          animation: auraSpin 6s linear infinite; filter: blur(10px); z-index: 1;
        }
        .profile-img-wrapper {
          position: relative; z-index: 2; width: 120px; height: 120px; border-radius: 50%;
          overflow: hidden; border: 3px solid rgba(${auraRgb}, 0.25);
          box-shadow: 0 10px 30px rgba(0,0,0,0.3);
          background-color: ${textColor === 'white' ? 'rgba(255,255,255,0.08)' : 'rgba(0,0,0,0.05)'};
          display: flex; justify-content: center; align-items: center;
        }

        /* 🔥 UPGRADE 3: The Floating Glass Arc */
        .glass-arc-footer {
          position: fixed; bottom: 25px; left: 50%; transform: translateX(-50%);
          display: flex; align-items: center; justify-content: center; gap: 8px;
          padding: 12px 24px; border-radius: 100px;
          background: ${footerBg}; border: 1px solid ${footerBorder};
          backdrop-filter: blur(24px); -webkit-backdrop-filter: blur(24px);
          z-index: 100; text-decoration: none; color: ${textColor};
          font-size: 11px; font-weight: 800; letter-spacing: 1.5px;
          box-shadow: 0 10px 30px -5px rgba(0,0,0,0.2); transition: all 0.3s cubic-bezier(0.25, 0.8, 0.25, 1);
        }
        .glass-arc-footer:hover { transform: translateX(-50%) translateY(-3px); box-shadow: 0 15px 35px -5px rgba(0,0,0,0.3); }
        .glass-arc-footer:active { transform: translateX(-50%) scale(0.96); }
      ...
      <div onClick={handleShare} className="share-trigger cascade-1">
      ...
      <main style={{ width: '100%', maxWidth: '400px', display: 'flex', flexDirection: 'column', alignItems: 'center', textAlign: 'center', zIndex: 10 }}>
        
        {/* Animated Aura + Profile Picture */}
        <div className="profile-aura-container cascade-1">
        ...
        {/* Name & Badge */}
        <div className="cascade-2" style={{ display: 'flex', alignItems: 'center', gap: '8px', marginBottom: '8px' }}>
        ...
        {/* Job Title & Bio */}
        <div className="cascade-3" style={{ width: '100%' }}>
        ...
        {/* Buttons & Links */}
        <div className="cascade-4" style={{ width: '100%', display: 'flex', flexDirection: 'column', gap: '16px' }}>
          {showContactBtn && (
      ...
      {/* 🔥 MARKETING: The Floating Glass Arc (Hidden for Premium Users) */}
      {profile.tier === 'free' && (
        <a href="/" className="glass-arc-footer cascade-5">
          <span>⚡ POWERED BY LINKSUPPLY</span>
        </a>
      )}
    </div>
  )

    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: '30px', width: '100%', animation: 'fadeIn 0.3s ease-in-out' }}>
      ...
      {isPremium ? (
        <>
                            ...
                            <div style={{ position: 'absolute', top: '-55px', backgroundColor: '#111', color: 'white', padding: '8px 12px', borderRadius: '8px', zIndex: 20, whiteSpace: 'nowrap', boxShadow: '0 4px 10px rgba(0,0,0,0.15)', animation: 'fadeIn 0.15s', display: 'flex', flexDirection: 'column', gap: '4px' }}>
                              ...
                              {(dataPoint.nfc !== undefined || dataPoint.qr !== undefined) && (
                                <div style={{ display: 'flex', gap: '8px', fontSize: '11px', fontWeight: '600' }}>
                                  {dataPoint.nfc > 0 && <span style={{ color: '#60a5fa' }}>📱 {dataPoint.nfc} NFC</span>}
                                  {dataPoint.qr > 0 && <span style={{ color: '#a78bfa' }}>📷 {dataPoint.qr} QR</span>}
                                </div>
                              )}
          ...
          {/* NEW: HARDWARE BREAKDOWN LEADERBOARD */}
          {stickers.length > 0 && (
            <div style={{ backgroundColor: 'white', padding: '30px', borderRadius: '16px', border: '1px solid #e5e7eb', boxShadow: '0 4px 6px -1px rgba(0, 0, 0, 0.05)', width: '100%' }}>
              <h2 style={{ fontSize: '20px', fontWeight: '800', color: '#111', margin: '0 0 5px 0', letterSpacing: '-0.5px' }}>Hardware Breakdown</h2>
              <p style={{ color: '#6b7280', fontSize: '14px', margin: '0 0 25px 0', fontWeight: '500' }}>Compare lifetime scans across your active tags</p>
              
              <div style={{ display: 'flex', flexDirection: 'column', gap: '18px' }}>
                {[...stickers].sort((a,b) => (b.tap_count || 0) - (a.tap_count || 0)).map((sticker, index) => {
                  const maxStickerTaps = Math.max(...stickers.map(s => s.tap_count || 0), 1);
                  // Ensure even 0 taps shows a tiny sliver so it's not totally invisible
                  const barWidth = Math.max(((sticker.tap_count || 0) / maxStickerTaps) * 100, 1);
                  
                  return (
                    <div key={sticker.id} style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', fontSize: '14px', fontWeight: '600' }}>
                        <div>
                          <span style={{ color: '#111' }}>{sticker.id}</span>
                          {sticker.tag_name && sticker.tag_name !== sticker.id && (
                            <span style={{ color: '#6b7280', fontWeight: '500', marginLeft: '6px' }}>({sticker.tag_name})</span>
                          )}
                        </div>
                        <span style={{ color: index === 0 && (sticker.tap_count || 0) > 0 ? '#10b981' : '#3b82f6', fontWeight: '800' }}>
                          {sticker.tap_count || 0} <span style={{ fontSize: '12px', fontWeight: '600', color: '#9ca3af' }}>scans</span>
                        </span>
                      </div>
                      <div style={{ width: '100%', backgroundColor: '#f3f4f6', height: '10px', borderRadius: '6px', overflow: 'hidden' }}>
                        <div style={{ 
                          width: `${barWidth}%`, 
                          backgroundColor: index === 0 && (sticker.tap_count || 0) > 0 ? '#10b981' : '#3b82f6', 
                          height: '100%', 
                          borderRadius: '6px', 
